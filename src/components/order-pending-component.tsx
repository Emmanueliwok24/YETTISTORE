...
export default function OrderPendingComponent() {
    ...
    const currentOrders = orders.filter(order => order.status === "pending").slice(indexOfFirstOrder, indexOfLastOrder);
    ...
    const handleNext = () => {
        if (currentPage < Math.ceil(orders.filter(order => order.status === "pending").length / ordersPerPage)) {
            setCurrentPage(prevPage => prevPage + 1);
        }
    };
        ...
        <React.Fragment>
            ...
            {orders.filter(order => order.status === "pending").length > ordersPerPage && (
                <nav className="flex justify-center items-center mt-4">
                    ...
                    <span className="mx-3">
                        Page {currentPage} of {Math.ceil(orders.filter(order => order.status === "pending").length / ordersPerPage)}
                    </span>
                    <button
                        onClick={handleNext}
                        disabled={currentPage === Math.ceil(orders.filter(order => order.status === "pending").length / ordersPerPage)}
                        className="px-3 py-1 rounded bg-gray-200 text-black disabled:opacity-50"
                    >
                        <ChevronRight />
                    </button>
