...
export default function OrderSuccessfulComponent() {
    ...
    const currentOrders = orders.filter(order => order.status === "confirmed").slice(indexOfFirstOrder, indexOfLastOrder);
    ...
    const handleNext = () => {
        if (currentPage < Math.ceil(orders.filter(order => order.status === "confirmed").length / ordersPerPage)) {
            setCurrentPage(prevPage => prevPage + 1);
        }
    };
    ...
    return (
        <React.Fragment>
            <div>
                <h1 className="w-full bg-[#35a4ff] text-white text-center p-2 font-bold mb-2">Successful Order</h1>
            </div>
            ...
            {orders.filter(order => order.status === "confirmed").length > ordersPerPage && (
                <nav className="flex justify-center items-center mt-4">
                    ...
                    <span className="mx-3">
                        Page {currentPage} of {Math.ceil(orders.filter(order => order.status === "confirmed").length / ordersPerPage)}
                    </span>
                    <button
                        onClick={handleNext}
                        disabled={currentPage === Math.ceil(orders.filter(order => order.status === "confirmed").length / ordersPerPage)}
                        className="px-3 py-1 rounded bg-gray-200 text-black disabled:opacity-50"
                    >
                        <ChevronRight />
                    </button>
