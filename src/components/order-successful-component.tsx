...
export default function OrderSuccessfulComponent() {
    ...
    return (
        <React.Fragment>
            <div>
                <h1 className="w-full bg-[#35a4ff] text-white text-center p-2 font-bold mb-2"> Success Order</h1>
            </div>
            {orders.filter(order => order.status === "confirmed").map((order) => (
                <Link  href={`/order/details/${order.order_number}`} key={order.order_number} className="flex items-start justify-between border rounded p-2 mb-3">
